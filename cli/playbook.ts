...
async function materialize(target: string, presetName: string) {
  ...
  const managedBlocks = {
    "AGENTS.md": await readFile(join(SOURCE_ROOT, "templates", "AGENTS.managed.md"), "utf-8"),
    "CLAUDE.md": await readFile(join(SOURCE_ROOT, "templates", "CLAUDE.managed.md"), "utf-8"),
  };
...
async function materializeVendorSkill(skill: VendoredSkill, name: string, availableSkills: string[], dst: string) {
  ...
  await writeFile(join(dst, "SKILL.md"), wrapper, "utf-8");
...
async function indexVendoredSkills(requireValidSnapshots: boolean) {
  ...
  for (const name of await findOwnedSkills())
    index.set(`owned:${name}`, { dir: join(SOURCE_ROOT, "skills", name), source: null as never, description: "" });
...
async function check(flags: Map<string, string | boolean>) {
  ...
  await checkConsumer(resolve(flagString(flags, "target", process.cwd())));
}
...
async function checkConsumer(target: string) {
  ...
  try {
    ...
    for (const name of config.modules) modules.push(await loadModule(name, stacksRoot));
  ...
  for (const file of ["AGENTS.md", "CLAUDE.md"]) {
    try {
      const block = managedBlock(await readFile(join(target, file), "utf-8"), file);

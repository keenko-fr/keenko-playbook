...
describe("Keenko migrations", () => {
  ...
  test("preserves a regex literal after contextual of with a Unicode for-of binding", async () => {
    const tree = createTreeWithEmptyWorkspace();
    await preset(tree, { name: "unicode_for_of_regex_text" });
    tree.write(".editorconfig", LEGACY_EDITORCONFIG);
    const loop = "for (const café of /project-_tabWidth-cache/.source) { void café; }";
    const customizedOxfmt = LEGACY_OXFMT_CONFIG.replace(
      'import ultracite from "ultracite/oxfmt";\n',
      `import ultracite from "ultracite/oxfmt";\n\n${loop}\n`
    );

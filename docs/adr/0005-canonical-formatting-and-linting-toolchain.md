...
- Canonical TypeScript scripts expose formatting, linting, typecheck, applicable tests, builds, deterministic generated-code verification, and a non-remediating merge-ready `check`; CI consumes those scripts and never fixes or pushes source. Fresh workspaces without a real test concern do not get a root `test` script.

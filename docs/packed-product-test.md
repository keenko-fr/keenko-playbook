...
Before published acceptance, `bun run release:wait-for-published -- <exact-version>` checks that exact package selector against `https://registry.npmjs.org` every 10 seconds. It makes at most 25 attempts and has a hard five-minute timeout. A different available version does not satisfy the check, and both temporarily missing metadata and transient registry lookup failures are retried within the same bound. Once the exact version resolves, the workflow passes it unchanged to `bun run test:published -- <exact-version>`.

If registry propagation exceeds the bound, the workflow fails with the exact selector, attempt count, timeout, and last lookup failure. Rerun the unchanged `bun run release:wait-for-published -- <exact-version>` and `bun run test:published -- <exact-version>` commands after npm propagation completes; do not rerun Nx Release or publication. If published acceptance exposes a product defect, the workflow fails visibly. The npm publication and release tag are immutable release events: do not unpublish, delete the tag, rewrite the release, or force-push history. Diagnose the consumer failure and ship a subsequent corrective release through the same Nx version-plan process.

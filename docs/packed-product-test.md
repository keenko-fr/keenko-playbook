...
The release workflow has one required `mode` choice: `rc` or `stable`. Both modes install from the lockfile, run `bun run check` (which includes `pack:check`), validate version plans, and run `bun run test:product` before invoking the same Nx Release architecture. RC mode runs `bun x nx release --yes --skip-publish --preid rc`, while the active version plan determines the semver bump, and then publishes the versioned package with `bun x nx release publish --tag rc`. A subsequent RC uses an appropriate `prerelease` plan and the same RC mode to increment the accepted RC line and remain on the explicit `rc` npm dist-tag. RC publication never owns or moves `latest`.

Stable mode runs `bun x nx release 1.0.0 --yes --skip-publish` without a prerelease flag, then publishes through `bun x nx release publish --tag latest`. Installed-Nx dry-run verification proves that this explicit target promotes an accepted `1.0.0-rc.N` to `1.0.0`; merely omitting `--preid rc` is not the promotion contract. Selecting stable and moving `latest` remain an explicit human KEE-5 decision: no RC automatically promotes, and the workflow remains manual. This release establishes the first supported project compatibility baseline and does not add a `0.x` to `1.0.0` consumer migration guarantee.

Nx remains the sole versioning, changelog, Git tag, push, GitHub release, and npm publication authority. `--skip-publish` suppresses only the top-level command's implicit npm publication; the explicit Nx publish phase publishes the already-versioned package and does not perform another version transition or consume another version plan. npm's default dist-tag behavior is not part of the release contract. After both Nx phases succeed, the workflow reads the version Nx wrote to the root `package.json`; it does not calculate a published version independently.

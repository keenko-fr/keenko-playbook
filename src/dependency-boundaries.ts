...
/* oxlint-disable effect/noAsyncFunction, effect/noNewError, effect/noNodeBuiltinImport, effect/noNullish, effect/noRuntimeTypeof, effect/noThrowStatement, effect/noUnknownParameters, typescript/no-unsafe-call, typescript/no-unsafe-member-access, typescript/strict-boolean-expressions -- This CLI boundary loads project-owned TypeScript policy and Nx's untyped runtime graph module, then reports verification failures synchronously to its caller. */
...
export const findDependencyBoundaryViolations = (
  ...
  for (const [source, dependencies] of Object.entries(graph.dependencies)) {
    ...
    const matchingConstraints = constraints.filter(({ sourceTag }) => sourceTags.includes(sourceTag));
    ...
    for (const { target } of dependencies) {
      ...
      for (const constraint of matchingConstraints) {
        if (constraint.onlyDependOnLibsWithTags.some((tag) => targetTags.includes(tag))) continue;
